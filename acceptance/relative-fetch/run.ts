/**
 * §5.3b — JS fetch() must resolve relative URLs against the document base.
 *
 * Pre-fix: fetch("/api/items.json") rejects with TypeError.
 * Post-fix: resolves against PAGE_URL and returns the JSON body.
 */
type CdpResult = { id?: number; error?: { message?: string }; result?: any };
const CDP = Number(process.argv[2] ?? 19339);
const MOCK = Number(process.argv[3] ?? 18098);
...
ws.addEventListener("open", async () => {
  try {
    ...
    await sleep(1500);
    const res = await race(send("Runtime.evaluate", { expression: "window.__res", returnByValue: true }), 5000);
    if (res === TIMED_OUT || (res as any).error) { step("relative fetch resolves", false, JSON.stringify(res)); return finish(); }
    step("fetch('/api/items.json') returned the JSON body", (res as any).result?.result?.value === '["alpha","beta"]',
      "value=" + JSON.stringify((res as any).result?.result?.value));
    return finish();
  } catch (e: any) { console.error("exception", e); return finish(); }
});

function finish() {
  console.log(`\n=== relative-fetch e2e: ${pass} PASS / ${fail} FAIL ===`);
