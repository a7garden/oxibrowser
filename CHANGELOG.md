...
- **HTTP/HTTPS/SOCKS proxy** — `BrowserConfig.proxy` (`Option<String>`) configures a proxy for all requests via `wreq::Proxy::all`; `serve --proxy <url>` CLI flag added. Supports `http://`, `https://`, `socks5://`, `socks5h://` (remote DNS).
