...
- **Viewport (device-metrics) emulation applied** — `Emulation.setDeviceMetricsOverride`/`clearDeviceMetricsOverride` now actually drive layout: subsequent navigations lay out at the overridden viewport (was ack-only / never consumed). Cross-thread `VIEWPORT_OVERRIDE` static in core; session `inject_dom_snapshot` reads it.
