...
- **Geolocation + timezone emulation** — `navigator.geolocation.getCurrentPosition` is now implemented, backed by `Emulation.setGeolocationOverride`/`clearGeolocationOverride` (returns POSITION_UNAVAILABLE without an override). `Emulation.setTimezoneOverride`/`clearTimezoneOverride` override the Intl/Date timezone. Overrides are cross-thread statics in core so the CDP layer (Emulation domain) sets them while the JS thread reads.
