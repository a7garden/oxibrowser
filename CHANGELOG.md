...
- **Automatic `Referer`** — requests now send a `Referer` computed per Chrome's default `strict-origin-when-cross-origin` policy (full URL same-origin, origin-only cross-origin, suppressed on https→http downgrade). The page origin is threaded to the fetch bridge via a `CURRENT_ORIGIN` thread-local + `FetchRequestMsg.origin`.
