...
- **Layout-based hit-testing** — `document.elementFromPoint(x, y)` now resolves the deepest element whose laid-out box (Blitz/Taffy `final_layout`) contains the point, replacing the DOM-order + tag-height heuristic. The `SetDocument` handler now builds the `DomSnapshot` from the render doc so node ids stay consistent with the layout boxes; the dead `estimate_element_height` heuristic was removed.
