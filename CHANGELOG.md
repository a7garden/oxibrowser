...
- **Request interception (live)** — `Fetch.requestPaused` is now actually emitted and awaited at the navigation boundary. `Fetch.enable` patterns are matched against the navigate URL; the request pauses until the client responds with `continue`/`fail`/`fulfill` (Playwright `page.route()`). `fulfill` mocks the page via a data-URL navigation. Previously `emit_request_paused` existed but was never called, so interception silently no-op'd.
