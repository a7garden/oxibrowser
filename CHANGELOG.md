...
- **File downloads** — navigating to a `Content-Disposition: attachment` response now saves the body to the configured download directory (`Page.setDownloadBehavior`) and emits `Page.downloadWillBegin` + `Page.downloadProgress` (state `completed`) via a new `CoreEvent::Download` bridged from the navigate path. Filename is parsed from Content-Disposition (quoted/unquoted) or the URL basename, sanitized against path traversal.
