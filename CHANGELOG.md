...
- **HTTP authentication (Basic + Digest)** — `BrowserConfig.http_username`/`http_password` + `HttpClient::request_with_auth` retry a request with credentials after a `401` + `WWW-Authenticate` challenge (`Basic` via base64, `Digest` MD5 + `qop=auth` per RFC 7616). The JS fetch bridge now routes through the auth-aware path. New `network::auth` module; quote-aware challenge parser.
