...
- **Streaming response bodies** — `read_body_limited` now streams the body chunk-by-chunk via `wreq`'s `stream` feature (`bytes_stream`), stopping at `max_bytes` instead of buffering the entire body first. Closes the OOM vector where a server omits/falsifies `Content-Length` (the old `bytes().await` + post-hoc truncate buffered everything). The wreq `stream` feature is now enabled.
